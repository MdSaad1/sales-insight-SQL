...
select   y.month_,y.year_,sum(y.sold_quantity*x.gross_price) as gross_price_by_month
from     fact_gross_price x
join     (select   month(date) as month_,year(date) as year_,product_code,sold_quantity,customer_code
          from     fact_sales_monthly
		  where    customer_code in (select customer_code from dim_customer where customer ="Atliq Exclusive")) y
on       x.product_code=y.product_code
group by month_
order by month_;

-- 8) quarterly sale quantity for the year of 2020. Result the 4th quarter has maximum number of sold_quantity
